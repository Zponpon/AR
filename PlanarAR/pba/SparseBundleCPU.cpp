...
void SparseBundleCPU<Float>::NormalizeDataD()
{
    ...
    if(__depth_scaling == 1.0f)
    {
		...
		if (__num_point_behind > 0 && __verbose_level >0)    std::cout << "WARNING: " << __num_point_behind << " points are behind camras.\n";
		if (__num_camera_modified > 0 && __verbose_level >0) std::cout << "WARNING: " << __num_camera_modified << " camera moved to avoid degeneracy.\n";
    }
}

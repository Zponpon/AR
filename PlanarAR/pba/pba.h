...
//filetype definitions for points and camera 定義座標點和攝影機的檔案型態
...
public:
    ////////////////////////////////////////////////////
    //methods for changing bundle adjustment settings 改變bundle adjustment設定
...
public:
    ...
    PBA_EXPORT virtual void SetCameraData(size_t ncam,  CameraT* cams);			//set camera data 設定攝影機數據
    PBA_EXPORT virtual void SetPointData(size_t npoint, Point3D* pts);			//set 3D point data 設定3D點數據
    PBA_EXPORT virtual void SetProjection(size_t nproj, 
        const Point2D* imgpts,  const int* point_idx, const int* cam_idx);		//set projections 設定2D投影點數據
